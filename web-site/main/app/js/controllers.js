...
angular.module('mpost.controllers', []).
    ...
    controller("CompanyCtrl", function ($scope, $http, Reseller, SaleChannel, Company, ComList, Supplier) {
        $scope.resellers = Reseller.query();
        $scope.saleChannels = SaleChannel.query();
        $scope.commissionLists = ComList.query();
        $scope.suppliers = Supplier.query();
        $scope.company = new Company();

        $scope.create = function () {
            if (!$('#create-company').valid()) return;
    ...
    controller("ShopCtrl", function ($scope, $http, Shop, Company) {
        $scope.shop = new Shop();

        $scope.create = function () {
            if (!$('#create-shop').valid()) return;

            $scope.shop.$save(function () {
                toastr.info("OK", "OK", {
                    "closeButton": true
                });
            });
    ...
    controller("ResellerCtrl", function ($scope, $http, Reseller) {
        $scope.reseller = new Reseller();
        $scope.resellers = Reseller.query();
        $scope.create = function () {
            if (!$('#update-reseller').valid()) return;

            $scope.reseller.$save(function () {
                toastr.info("OK", "OK", {
                    "closeButton": true
                });
            });
    ...
    controller("ContactCtrl", function ($scope, $http, Contact) {
        $scope.contact = new Contact();

        $scope.create = function () {
            if (!$('#create-contact').valid()) return;
    ...
    controller("DeliveryCtrl", function ($scope, $http, Documents) {
        $scope.documents = new Documents();

        $scope.create = function () {
            if (!$('#create-delivery-stt').valid()) return;

            $scope.documents.$save(function () {
                toastr.info("OK", "OK", {
                    "closeButton": true
                });
            });
    ...
    controller("CloseConventionCtrl", function ($scope, $http, Shop, Terminal, ClosedConvention) {
        $scope.shops = Shop.query();
        $scope.terminals = Terminal.query();
        $scope.closeConventions = new ClosedConvention();

        $scope.close = function () {
            if (!$('#close-convention').valid()) return;

            $scope.closeConventions.$save(function () {
                toastr.info("OK", "OK", {
                    "closeButton": true
                });
            });
    ...
    controller('TerminalCtrl', function ($scope, $rootScope, $modal, Terminal, TerminalType, TerminalStock, Service) {
        $scope.terminalStocks = TerminalStock.query();
        $scope.terminalTypes = TerminalType.query();
        $scope.services = Service.query();

        $scope.terminal = new Terminal();

        $scope.openModal = function (serviceActive, modal_id, modal_size, modal_backdrop) {
            $rootScope.currentModal = $modal.open({
                templateUrl: modal_id,
                size: modal_size,
                backdrop: modal_backdrop
            });
            $rootScope.serviceActive = serviceActive;
            $rootScope.activeService = function () {

            }
        };
    }).
    controller('TerminalStockCtrl', function ($scope, $http, TerminalStock) {
        $scope.telcoSuppliers = TerminalStock.query();
        $scope.terminalStock = new TerminalStock();

        $scope.create = function () {
            if (!$('#create-terminal-stock').valid()) return;

            $scope.terminalStock.$save(function () {
                toastr.info("OK", "OK", {
                    "closeButton": true
                });
            });
    ...
    controller('ShopManagerCtrl', function ($scope, $http, ShopManager) {
        $scope.contactTypes = ShopManager.query();
        $scope.shopManager = new ShopManager();

        $scope.create = function () {
            if (!$('#create-shop-manager').valid()) return;

            $scope.shopManager.$save(function () {
                toastr.info("OK", "OK", {
                    "closeButton": true
                });
            });
    ...
    controller('GABCtrl', function ($scope, $http, GAB) {
        $scope.gab = new GAB();

        $scope.create = function () {
            if (!$('#create-gab').valid()) return;

            $scope.gab.$save(function () {
                toastr.info("OK", "OK", {
                    "closeButton": true
                });
            });
    ...
    controller('UIModalsCtrl', function ($scope, $rootScope, $modal, $sce) {
        // Open Simple Modal
        $scope.openModal = function (service, modal_id, modal_size, modal_backdrop) {
            $rootScope.currentModal = $modal.open({
                templateUrl: modal_id,
                size: modal_size,
